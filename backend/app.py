...
def load_model():
    ...
    tokenizerfile = ARTEFACTS / "tokenizer.json"
    if not tokenizerfile.exists():
        raise FileNotFoundError(f"Tokenizer file not found at {tokenizerfile}")
    tokenizer = Tokenizer.from_file(str(tokenizerfile))
    configfile = ARTEFACTS / "config.json"
    if not configfile.exists():
        raise FileNotFoundError(f"Config file not found at {configfile}")
